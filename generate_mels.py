...
def GTA_Synthesis(output_directory, checkpoint_path, n_gpus,
    ...
    for i, batch in enumerate(train_loader):
        ...
        for k in range(batch_size):
            ...
            mel_paths.append(hparams.output_dir + d.replace('.wav',''))
        ...
        for k in range(batch_size):
            ...
            end = torch.tensor(output_lengths[k]).int() #torch.tensor(output_lengths[k]-diff).int()
            k = torch.tensor(k).int()
            mel = mel_outputs_postnet[k,:,:end]
